...
if hash lsb_release 2>/dev/null; then
	...
	if [ $ubuntu != "14.04" ]; then
		echo "# - WARNING : dev-tools is currently only tested on Ubuntu 14.04, YMMV. Please open an issue if you've used it successfully on another version of Ubuntu."
	else
		echo "# - fully supported host detected: Ubuntu 14.04"
	fi
else
